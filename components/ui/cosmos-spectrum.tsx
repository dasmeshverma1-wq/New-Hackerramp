...
export function CosmicSpectrum({
  ...
  const colorThemes = {
    ...
    wit: ["#06050c", "#4A2FD4", "#7B5CFF", "#AE33FF", "#FF1FC0", "#FF3F6C", "#00E5FF", "#FFC0FD"],
  }

  const darkThemes = ["blue-black", "beige-black", "monochrome", "wit"]
      ...
      try {
        ...
        window.setTimeout(() => {
          if (window.gsap && window.ScrollTrigger) {
            ...
            setupAnimations(containerRef.current)
          }
        }, 100)
      } catch (error) {
  ...
  return (
    <div ref={containerRef} className="cosmos-spectrum relative min-h-screen overflow-x-hidden">
      <section className="flex h-screen w-full flex-col justify-center p-8">
        <h1 className="hero-title text-center text-5xl font-bold tracking-tighter transition-colors duration-300 md:text-7xl">
          {splitText(title)}
        </h1>
      </section>

      <div className="scroll-hint nav-bottom-center pointer-events-auto fixed bottom-8 left-1/2 z-[1000] -translate-x-1/2 text-xs uppercase tracking-wide transition-colors duration-300">
...
function setupAnimations(root: HTMLElement | null) {
  ...
  const titleChars = scope.querySelectorAll(".hero-title .char")
  ...
  const scrollHintChars = scope.querySelectorAll(".scroll-hint .char")
  ...
  const animationSection = scope.querySelector(".animation-section")
  if (!animationSection) return

  const mainTitle = scope.querySelector(".main-title")
  ...
  tl.to(scope.querySelector(".svg-container"), { opacity: 1, duration: 0.01 }, 0)
    .to(scope.querySelector(".main-title"), { opacity: 1, duration: 0.01 }, 0)
    .to(scope.querySelector(".svg-container"), {
    ...
    .to(scope.querySelector(".scroll-hint"), {
      opacity: 0,
      duration: 0.6,
      ease: "power2.out",
    }, 0.2)
